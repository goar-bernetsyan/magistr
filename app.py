...
def deleting_columns(df, params): # Удаление столбцов (например если в ней слишком много NaN или этот столбец не влияет на обучение)
    ...
    return f'<h5>Удалены столбцы : {", ".join([f'"{c}"' for c in col])}</h5>' + df.head(5).to_html(classes='data-table', index=False)
...
def data_cleaning(df, params): # Очистка данных (от NaN)
    ...
    # выбор для числовых столбцов : заполнение предыдущим/следующим значением / заменить на самое частое значение ? / заменить на 0 / на среднее / на медиану / предсказать значение (только для числовых)
    # выбор для строковых столбцов : заполнение предыдущим/следующим значением / заменить на самое частое значение / на новую категорию 'unknown' / на моду
    # выбор для столбцов даты и времени : заполнение предыдущим/следующим значением (сначала все временные столбы разделяем на несколько нормальных (в одной денб в другой меся ит п)) так что отдельно пункт для временных нам не нужен
